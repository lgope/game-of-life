    ...
    for (let y = 0; y < rows; y++) {
      for (let x = 0; x < cols; x++) {
        ...
        if (gameBoard.current[y][x]) {
          if (neighbour === 2 || neighbour === 3) { // Any live cell with two or three live neighbours survives.
            newBoard[y][x] = true;
          } else {
            newBoard[y][x] = false;
          }
        } else {
          if (!gameBoard.current[y][x] && neighbour === 3) { // Any dead cell with three live neighbours becomes a live cell.
            newBoard[y][x] = true;
          }
        }
      }
